...
num_time_steps = 2000; % SET NUMBER OF LEVEL SET METHOD TIMESTEPS
...
phi = create_circle(X, Y, [0 0], 0.5);
phi1 = create_circle(X, Y, [0,0], 0.3);
phi1 = combine_phi(phi1, phi1, 'complement');
phi = combine_phi(phi, phi1, 'intersection');
phi1 = create_circle(X, Y, [0.4, 0], 0.1);
phi = combine_phi(phi, phi1, 'difference');
